	...
	class Auth{
		...
		public static function registerAccount($userId, $username, $email, $password, $approvalNeeded){
			if($userExists($userid)){
				...
				$res = DB::query("insert into account(userid, username, email, password, erstellungsdatum) values :i,:u,:e,:p,:d;",[':i'=>$userid,':u'=>$username,':e'=>$email,':d'=>$date,':p'=>password_hash($password, PASSWORD_DEFAULT)]);				if($approvalNeeded){
